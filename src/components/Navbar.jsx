...
export function NavbarDefault() {
    ...
    const navList = (
        <ul className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
            <Typography as="li" variant="small" color="blue-gray" className="flex items-center gap-x-2 p-1 font-medium cursor-pointer" onClick={() => navigate('/browse')}>
            ...
            <Typography as="li" variant="small" color="blue-gray" className="flex items-center gap-x-2 p-1 font-medium cursor-pointer" onClick={() => navigate('/mycourses')} >
            ...
            <Typography as="li" variant="small" color="blue-gray" className="flex items-center gap-x-2 p-1 font-medium cursor-pointer" onClick={() => navigate('/publishes')}>
                <svg fill="#889ba5" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                    width="16" height="17" viewBox="0 0 256 253" >
    ...
    return (
        <Navbar className="sticky top-0 mx-auto max-w-screen-xl px-4 py-2 lg:px-8 lg:py-4 rounded-lg shadow-md bg-transparent backdrop-blur-lg text-slate-100 mb-4">
            <div className="container mx-auto flex items-center justify-between text-blue-gray-900">
                <Typography as="a" className="mr-4 cursor-pointer py-1.5 from-neutral-800 font-bold text-4xl" onClick={() => navigate('/')}>
                    Upgrade
                </Typography>
                <div className="hidden lg:block">{navList}</div>
                {
                    token == null ? <div className="flex items-center gap-x-1">
                        <Button variant="text" size="sm" className="hidden lg:inline-block bg-inherit border text-white" onClick={() => navigate('/login')}>
                            <span>Log In</span>
                        </Button>
                        <Button variant="gradient" size="sm" className="hidden lg:inline-block text-slate-800 bg-white" onClick={() => navigate('/signup')} >
                            <span>Sign Up</span>
                        </Button>
                    </div> :
                        <Button variant="gradient" size="sm" className="hidden lg:inline-block text-slate-800 bg-white" onClick={() => {
                    ...
                    {navList}
                    {
                        token == null ?
                            <div className="flex items-center gap-x-1">
                                ...
                                <Button color="white" fullWidth variant="gradient" size="sm" className="rounded-full" onClick={() => navigate('/signup')} >
                                    <span className="text-black">Sign Up</span>
                                </Button>
                            </div> :
                            <Button color="white" fullWidth variant="gradient" size="sm" className="rounded-full" onClick={() => {
                                localStorage.removeItem('token');
                                navigate('/browse');
                                window.location.reload();
                            }} >
